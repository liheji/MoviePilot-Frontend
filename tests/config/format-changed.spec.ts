...
function createRepository() {
  ...
  git(repository, 'init', '--initial-branch=lite')
...
describe('Prettier 变更文件选择器', () => {
  it('按 base 与 head 选择受支持文件并排除删除、忽略和符号链接', () => {
    ...
    expect(listSelected(repository, '--base', 'lite', '--head', 'HEAD')).toEqual([
      'src/line\nbreak.ts',
      'src/modified.ts',
      'src/new file.vue',
      'src/rename [new].ts',
  ...
  it('首个候选没有 merge base 时回退到后续可比较分支', () => {
    ...
    git(repository, 'update-ref', 'refs/remotes/upstream/lite', unrelatedCommit)
    git(repository, 'update-ref', 'refs/remotes/origin/lite', baseCommit)
  ...
  it('纳入转为普通文件的类型变更，并继续排除最终为符号链接的路径', () => {
    ...
    expect(listSelected(repository, '--base', 'lite', '--head', 'HEAD')).toEqual(['src/to-regular.ts'])
  })
